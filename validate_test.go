...
// TestValidate_DeprecatedViolationsMethod tests the deprecated Violations() accessor.
func TestValidate_DeprecatedViolationsMethod(t *testing.T) {
	inv := &Invoice{
		Profile:             CProfileEN16931,
		InvoiceNumber:       "", // BR-2 violation
		InvoiceTypeCode:     380,
	...
	// Test deprecated Violations() method still works
	violations := inv.Violations()
	if len(violations) == 0 {
		t.Error("Deprecated Violations() method returned empty slice")
	}

	// Verify it returns a copy (modifying shouldn't affect internal state)
	originalCount := len(violations)
	violations[0].Rule = "MODIFIED"

	// Call Violations() again - should return fresh copy
	violations2 := inv.Violations()
	if violations2[0].Rule == "MODIFIED" {
		t.Error("Violations() did not return a copy - internal state was modified")
	}

	// Count should be same
	if len(violations2) != originalCount {
		t.Errorf("Second call to Violations() returned different count: %d vs %d", len(violations2), originalCount)
	}
}
